use ethers::{
    prelude::abigen,
    providers::Provider,
    types::{Address, U256}, utils::{format_units},
};
use std::sync::Arc;
...
async fn main() -> eyre::Result<()> {
    ...
    let erc20_addresses_raw: [&str; 2] = ["0xdAC17F958D2ee523a2206206994597C13D831ec7", "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48"];
    ...
    for erc20_addr in erc20_addresses {
        ...
        let balance: U256 =
        erc20.balance_of(user).call().await?;

        let decimals = erc20.decimals().call().await? as i32;

        println!("the {}(erc20) balance_of {}(user): {:?}", erc20_addr, user, format_units(balance, decimals).unwrap().separate_with_commas());
    }
